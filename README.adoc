...
This `settings.gradle` file isn't required for a single-module Gradle project but without this definition, the project name will default to the name of the folder in which it is contained (`guide-gradle-intro`).
...
So what's not configured? By default, when the `liberty` and the `war` plugins are applied, the default `war` is installed to the `apps` folder.
...
The `-i` option provides additional information in the terminal.
